function [RC,add_on,EL,VaR] = add_on_Approach_tStudent(flag,simulated,fix_or_sim,M,RC_naive,EL_naive,alpha,nu)
%[RC,add_on,EL,VaR] = add_on_Approach_tStudent(flag,simulated,fix_or_sim,M,
% RC_naive,EL_naive,alpha,nu)
...
EL_conditional=LGD.*tcdf((k-sqrt(rho).*M)./sqrt(1-rho),nu);  % Computing EL conditional via Double t-distribution
